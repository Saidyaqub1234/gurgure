...
import { adminTeam, uploadFile } from '@/api';
import type { TeamMember } from '@/types';

const emptyForm = { name: '', slug: '', position: '', bio: '', email: '', phone: '', order: 0, is_published: true };
...
export default function AdminTeam() {
  ...
  const openCreate = () => { setEditing(null); setForm({ ...emptyForm, order: items.length + 1 }); setPhotoFile(null); setShowModal(true); };

  const openEdit = (item: TeamMember) => {
    setEditing(item);
    setForm({ translations: ((item) as any).translations || {},
      name: item.name, slug: item.slug, position: item.position, bio: item.bio,
      email: item.email, phone: item.phone, order: item.order, is_published: item.is_published,
    });
    setPhotoFile(null);
    setShowModal(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editing) {
        await adminTeam.update(editing.id, payload);
        toast.success('Team member updated');
      } else {
        await adminTeam.create(payload);
    ...
    <AdminLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-heading font-bold text-white">Team</h1>
            <p className="text-white/40 mt-1">Manage team members</p>
          </div>
          <button onClick={openCreate} className="btn-primary text-sm px-5 py-2.5">+ New Member</button>
        </div>
        ...
        <div className="admin-table">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-12 text-center text-white/30">No team members found</td></tr>
                ) : items.sort((a, b) => a.order - b.order).map((item) => (
                  <tr key={item.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        {item.photo && (
                          <img src={item.photo} alt={item.name} className="w-10 h-10 rounded-full object-cover bg-white/5" />
                        )}
                        <div>
                    ...
                    <td className="text-sm text-white/50">{item.position}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowModal(false)}>
          <div className="admin-modal max-w-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-heading font-bold text-white">{editing ? 'Edit Team Member' : 'New Team Member'}</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input className="admin-input" value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} placeholder="member-slug" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
              ...
              <div>
                ...
                <textarea className="admin-input" value={form.bio} onChange={e => setForm({ ...form, bio: e.target.value })} placeholder="Short biography" rows={3} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <input type="tel" className="admin-input" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="+93 xxx xxx xxx" />
                </div>
              </div>
              <div>
                <label className="admin-label">Photo</label>
                <input type="file" accept="image/*" className="admin-input" onChange={e => setPhotoFile(e.target.files?.[0] || null)} />
              </div>
              <div className="flex items-center gap-3">
