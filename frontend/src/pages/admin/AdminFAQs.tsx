...
import { adminFAQs } from '@/api';
import type { FAQ } from '@/types';

const emptyForm = { question: '', answer: '', category: '', order: 0, is_published: true };
...
export default function AdminFAQs() {
  ...
  const openEdit = (item: FAQ) => {
    ...
    setForm({ translations: ((item) as any).translations || {},
      ...
      category: item.category, order: item.order, is_published: item.is_published,
    });
    setShowModal(true);
  };
    ...
    <AdminLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openCreate} className="btn-primary text-sm px-5 py-2.5">+ New FAQ</button>
        </div>
        ...
        <div className="admin-table">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-12 text-center text-white/30">No FAQs found</td></tr>
                ) : items.sort((a, b) => a.order - b.order).map((item) => (
                  <tr key={item.id}>
                    <td>
                      <p className="font-medium text-white/80 max-w-md truncate">{item.question}</p>
                    </td>
                    <td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowModal(false)}>
          <div className="admin-modal max-w-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-heading font-bold text-white">{editing ? 'Edit FAQ' : 'New FAQ'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="admin-input" value={form.question} onChange={e => setForm({ ...form, question: e.target.value })} placeholder="Frequently asked question" />
              </div>
              <div>
                <label className="admin-label">Answer</label>
                <textarea className="admin-input min-h-[150px]" value={form.answer} onChange={e => setForm({ ...form, answer: e.target.value })} placeholder="Answer to the question" />
              </div>
